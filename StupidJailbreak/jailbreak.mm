...
kern_return_t cb(task_t tfp0_, kptr_t kbase, tihmstar::offsetfinder64 *fi){
    resume_all_threads();
    LOG("done exploit!\n");
    tfp0 = tfp0_;
...
//    }
    ...
    // Do what v0rtex would have done with the callback
    if (cb(kernel_task, kbase, &fi) != KERN_SUCCESS) {
        resume_all_threads();
        progressBar.hidden = YES;
        statusLabel.text = @"KPP bypass failed, or my own stupidity";
